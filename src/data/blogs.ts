...
export interface BlogPost {
  ...
  // expects a Date type. Nullable for posts authored before the ISO
  // column existed in the schema, even though the 7 seeded rows all
  // have it. Renderer falls back to `date` when this is null.
...
// order shown in the BlogBrowser filter strip. "ALL" is a UI sentinel for
// the filter strip's "show every post" state and is NOT a valid value for
// a real post's `category` field; the form/Zod use BLOG_FORM_CATEGORIES.
...
// Canonical category list the admin BlogEditor dropdown binds to, and that
// the Zod blogInsertSchema validates against. Mirrors blogCategories minus
// the "ALL" sentinel. Adding a category here makes it immediately
// selectable in the admin form AND accepted by the server-side validator -
// no schema migration needed (the legacy DB check constraint was dropped
// in migration 0008).
export const BLOG_FORM_CATEGORIES = blogCategories.filter((c) => c !== "ALL");
...
export const blogs: BlogPost[] = [
  {
    id: "tour-operator-reviews-google-ads-case-study",
    slug: "tour-operator-reviews-google-ads-case-study",
    title: "How a Coastal Tour Operator Tripled Direct Bookings",
    category: "Local SEO",
    author: "Syed Belal",
    date: "June 26, 2026",
    datePublishedISO: "2026-06-26",
    readTime: "9 min read",
    image: "/blog/tour-operator-reviews-google-ads-case-study/header.png",
    excerpt:
      "A coastal tour operator went from invisible in Google's map pack to 3.4x more direct bookings in 120 days. The reviews-and-ads playbook, by the numbers.",
    content: `
...
If you run a tour, charter, or activity business and your phone isn't ringing the way your tours deserve, this is usually why — and it's fixable in a quarter. [See how our Pipeline program builds this engine for local operators](/programs), or [book a free 20-minute teardown of your Google presence](/audit) and we'll show you where the bookings are leaking.
    ...
    faqs: [
      {
        q: "Do online reviews really affect Google ranking?",
        a: "Yes. Review count, average star rating, and how recent your reviews are all feed local-pack (map) ranking, and a higher rating also lifts click-through and on-page conversion. Reviews are one of the strongest local ranking factors you actually control.",
      },
      {
        q: "How fast can a tour operator get more reviews?",
        a: "Fast. With an automated text-and-email request sent within an hour of the tour, while the experience is still fresh, review velocity commonly rises 3-5x within the first few weeks. The bottleneck is almost always asking, not willingness.",
      },
      {
        q: "Are Google Ads worth it for a small tour operator?",
        a: "Usually yes, if you target high-intent local searches and track actual bookings. Travel and activity campaigns can return several dollars per dollar spent, but only with conversion tracking and tight geo and keyword targeting. Without tracking, you are guessing.",
      },
      {
        q: "Why push direct bookings instead of relying on Viator or GetYourGuide?",
        a: "OTAs typically take roughly 20-30% commission per booking. Shifting even part of your volume to direct booking recovers that margin, and it hands you the customer relationship, the email, and the data to remarket later.",
      },
      {
        q: "How long before this shows up in revenue?",
        a: "Review counts and ad traffic move within weeks. Map-pack ranking and direct-booking share build over roughly 60-120 days as trust and visibility compound. Treat it as a quarter-long play, not a one-week switch.",
      },
    ],
  },
  {
