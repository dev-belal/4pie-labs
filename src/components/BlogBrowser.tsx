    ...
    <>
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8 text-foreground">
        ...
        <div className="relative w-full md:w-80 group">
          <Search className="absolute left-5 top-1/2 -translate-y-1/2 w-4 h-4 text-foreground/30 group-focus-within:text-primary transition-colors" />
          <input
            type="text"
          ...
          {filteredPosts.length > 0 ? (
            filteredPosts.map((blog, i) => (
              ...
              >
                <Link
                  href={`/blog/${blog.slug}`}
                  className="group glass-morphism rounded-[40px] border-foreground/5 hover:border-foreground/20 transition-all overflow-hidden flex flex-col h-full shadow-2xl"
                >
